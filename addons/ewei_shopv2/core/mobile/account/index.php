...
class Index_EweiShopV2Page extends MobilePage 
{
	...
	protected function rf($type) 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (empty($member)) 
			{
				$memberag = pdo_fetch('select id,openid,mobile,pwd,agenttime,isagent,salt from ' . tablename('ewei_shop_member') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $agentid, ':uniacid' => $_W['uniacid']));
				if (empty($memberag)&&($agentid > 0)) 
				{
					show_json(0, '您填写的邀请码不存在，如无邀请人可留空');
				}
				if(!empty($memberag)){
					if ($memberag['isagent']<1) 
					{
						show_json(0, '您填写的邀请码所有者为成为分销商！');
					}
					if($agentid > 1){
					$data['agentid'] = $agentid;
					}
					pdo_insert('ewei_shop_member', $data);
					$memberx = pdo_fetch('select id,openid,mobile,pwd,salt from ' . tablename('ewei_shop_member') . ' where mobile=:mobile and uniacid=:uniacid limit 1', array(':mobile' => $mobile, ':uniacid' => $_W['uniacid']));
					
					
					p('commission')->saveRelation($memberx['id'], $agentid);
				}else{
					pdo_insert('ewei_shop_member', $data);
				}
	...
	public function orderstatus()
	{
        ...
        global $_GPC;
        ...
        }else if(strpos($_GPC["ordersn"],'MC') !==false){
          if($paytype == 21){
            $type = 'wechat';
          }else{
          	 $type = 'alipay';
          }
          	p('membercard')->payResult($_GPC["ordersn"], $type, is_h5app() ? true : false);
			$result = pdo_update('ewei_shop_member_card_order', array('paytype' => $type, 'status' => 1, 'paytime' => time(), 'finishtime' => time(), 'apppay' => is_h5app() ? 1 : 0), array('orderno' => $_GPC["ordersn"]));
			echo json_encode($result) ;
		}else if(strpos($_GPC["ordersn"],'PT') !==false){
		 ...
		 WHERE `uniacid`=:uniacid AND `module`=:module AND `tid`=:tid limit 1', array(':uniacid' => $uniacid, ':module' => 'groups', ':tid' => $_GPC["ordersn"]));
		 $orderid = pdo_fetchcolumn('select id from ' . tablename('ewei_shop_groups_order') . ' where orderno=:orderno and uniacid=:uniacid and openid=:openid limit 1', array(':orderno' => $_GPC["ordersn"], ':uniacid' => $uniacid, ':openid' => $log['openid']));

		  p('groups')->payResult($log['tid'], $type, is_h5app() ? true : false);
				$result =	pdo_update('ewei_shop_groups_order', array('pay_type' => 'wechat', 'status' => 1, 'paytime' => time(), 'starttime' => time(), 'apppay' => is_h5app() ? 1 : 0), array('id' => $orderid));

				echo json_encode($result) ;
			
			
		}else if(strpos($_GPC["ordersn"],'EE') !==false){
          if($paytype == 21){
            $type = 'wechat';
          }else{
          	 $type = 'alipay';
		  ...
		  $uniacid = $_W['uniacid'];
		    ...
		    WHERE `uniacid`=:uniacid AND `logno`=:logno limit 1', array(':uniacid' => $uniacid, ':logno' => $_GPC["ordersn"]));
			$member = m('member')->getMember($log['openid']);
			$goods = p('creditshop')->getGoods($log['goodsid'], $member, $log['optionid'], $log['goods_num']);
			
          	p('creditshop')->payResult($_GPC["ordersn"], $type,$goods['money'] + $goods['dispatch'], is_h5app() ? true : false);
		...
		}else
        {
		  ...
		  //打印和消息通知
		  $order = pdo_fetch("SELECT * FROM " . tablename("ewei_shop_order") . " WHERE `ordersn`=:ordersn limit 1", array( ":ordersn" => $_GPC["ordersn"] ));
