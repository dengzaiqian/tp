...
class User_EweiShopV2Page extends PluginWebPage
{
	...
	protected function post()
	{
		...
		$item = pdo_fetch('select * from ' . tablename('ewei_shop_merch_user') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
                if (!empty($item)) {
                    $item['shequids'] = explode(',', $item['shequids']);
                }else{
                    $item['shequids']=array();
                }
		...
		$category = $this->model->getCategory();
                ...
                $shequs = pdo_fetchall('select * from ' . tablename('ewei_shop_shequ_user') . ' where uniacid=:uniacid and status=:status', array(':uniacid' => $_W['uniacid'], ':status' => 1));
