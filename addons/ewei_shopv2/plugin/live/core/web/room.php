...
class Room_EweiShopV2Page extends PluginWebPage
{
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if ($data['livetype'] == 2) {
				if (!empty($data['video'])) {
                                        if($data["liveidentity"]=="huajiao"){
                                            $check=strstr($data['video'], 'm3u8');
                                            if(!$check){
                                            $getVideo = $this->model->getLiveInfoNow($data['video'], "auto");
                                            if(!empty($getVideo["hls_url"])){
                                                $data['video']=$getVideo["hls_url"];
                                                if($data["covertype"]==0){
                                                    $data['thumb']=$getVideo["poster"];
                                                }
                                            }else{
                                                show_json(0, '获取房间信息失败！');
                                            }
                                        }
                                        }
					if (!$this->check_url($data['video'])) {
                                                $data['video']=$_W['siteroot']."/attachment/".$data['video'];
                                                if(!$this->check_url($data['video'])){
	...
	array('id' => 0, 'key' => 'default', 'levelname' => empty($_W['shopset']['shop']['levelname']) ? '默认会员' : $_W['shopset']['shop']['levelname'])
		...
		$liveidentity = array(
			...
			array('type' => 1, 'identity' => 'inke', 'name' => '映客直播'),
                        array('type' => 2, 'identity' => 'huajiao', 'name' => '花椒直播'),
