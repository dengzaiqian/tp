...
class LiveModel extends PluginModel 
{
	...
	public function getWsAddress() 
	{
        ...
        //获取直播真实地址（目前只有花椒）
        public function getLiveInfoNow($url = NULL, $type = 'auto'){
            if (empty($url)) {
			return error(1, '视频地址为空');
            }
		...
		case 'huajiao':
			...
			$roomid = $matchs[1];
                        $tt = time();
                        $room_url = 'https://h.huajiao.com/api/getFeedInfo?sid='.$tt.'&liveid='.$roomid;
                        $apiResult = $this->curl_get($room_url);
                        if($apiResult){
                            $apiResult =  json_decode($apiResult,true);
                            $feedInfo=$apiResult['data']['feed']['feed'];
                            if($apiResult["errno"]==0&&!empty($apiResult["data"]["live"]["main"])){
                                $resultArr = array('status' => $feedInfo['paused'] == 'N' ? 1 : 0, 'poster' => $feedInfo['image'], 'hls_url' =>$apiResult["data"]["live"]["main"]);
                            }
                        }
                        break;
            }
