define(['core', 'tpl', 'biz/plugin/diyform'], function (core, tpl, diyform) {
    ...
    modal.init = function (params) {
        ...
        $('.btn-submit').click(function () {
            ...
            if ($('#shequname').isEmpty()) {
                FoxUI.toast.show('请填写社区名称!');
                return
            }
            if ($('#salecate').isEmpty()) {
            ...
            if ($(".diyform-container").length > 0) {
                diyformdata = diyform.getData('.page-shequ-register .diyform-container');
                if (!diyformdata) {
                    return
                }
            ...
            core.json('shequ/register', data, function (pjson) {
                if (pjson.status == 0) {
