...
class Category_EweiShopV2Page extends PluginWebPage
{
	...
	protected function post()
	{
		...
		$item = pdo_fetch('select * from ' . tablename('ewei_shop_shequ_category') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		include $this->template();
	}
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,catename FROM ' . tablename('ewei_shop_shequ_category') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);
	...
	protected function post_swipe()
	{
		...
		$item = pdo_fetch('select * from ' . tablename('ewei_shop_shequ_category_swipe') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		include $this->template();
	}
	...
	public function delete_swipe()
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('ewei_shop_shequ_category_swipe') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);
