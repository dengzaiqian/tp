...
class Check_EweiShopV2Page extends PluginWebPage
{
    protected function main($status = 1)
    {
        ...
        include($this->template("shequ/check/index"));
    }
    ...
    public function detail()
    {
        ...
        {
            $data = $this->model->getshequPriceList($item["shequid"], $orderid, 10);
            if( !empty($data) ) 
            {
                $flag = 1;
                if( !empty($keyword) ) 
    ...
    public function confirm()
    {
        ...
        if( $type == 1 ) 
        {
            ...
            $pass_data = $this->model->getPassApplyPrice($item["shequid"], $bpid);
            ...
            foreach( $orderids as $key => $orderid ) 
            {
                ...
                {
                    pdo_update("ewei_shop_order", array( "shequapply" => 2 ), array( "id" => $orderid ));
                }
                else
                {
                    pdo_update("ewei_shop_order", array( "shequapply" => -1 ), array( "id" => $orderid ));
                }
