...
class Credit_EweiShopV2Page extends PluginWebPage
{
    protected function main($status = 1)
    {
        ...
        if( !empty($searchfield) && !empty($keyword) ) 
        {
            ...
            else
            {
                ...
                {
                    $condition .= " and ( u.shequname like :keyword or u.mobile like :keyword or u.realname like :keyword)";
                }
        ...
        $sql = "select b.*,u.shequname,u.realname,u.mobile from " . tablename("ewei_shop_shequ_bill") . " b " . " left join " . tablename("ewei_shop_shequ_user") . " u on b.shequid = u.id" . " where 1 " . $condition . " ORDER BY b.id desc ";
        ...
        $_sql = "select iscreditmoney,creditrate from " . tablename("ewei_shop_shequ_user") . "where 1 " . $_condition;
        $_params = array( ":uniacid" => $_W["uniacid"], ":id" => $shequid );
        ...
        if( $_GPC["export"] == "1" ) 
        {
            ...
            unset($row);
            $columns = array(  );
            $columns[] = array( "title" => "商城信息", "field" => "shequname", "width" => 12 );
        ...
        $total = pdo_fetchcolumn("select COUNT(1) from " . tablename("ewei_shop_shequ_bill") . " b " . " left join " . tablename("ewei_shop_shequ_user") . " u on b.shequid = u.id" . " where 1 " . $condition, $params);
        $pager = pagination2($total, $pindex, $psize);
        $groups = $this->model->getGroups();
        include($this->template("shequ/credit/index"));
    }
    ...
    public function detail()
    {
        ...
        {
            $shequ_user = pdo_fetch("select * from " . tablename("ewei_shop_shequ_user") . " where uniacid=:uniacid and id=" . $item["shequid"], array( ":uniacid" => $_W["uniacid"] ));
            if( !empty($shequ_user["payopenid"]) ) 
            {
                $member = m("member")->getMember($shequ_user["payopenid"]);
            }
        ...
        {
            $data = $this->model->getshequCreditList($item["shequid"], $orderid, 10, $item["creditrate"], $item["isbillcredit"]);
            if( !empty($data) ) 
            {
                $flag = 1;
                if( !empty($keyword) ) 
        ...
        if( $_GPC["export"] == "1" ) 
        {
            ...
            $columns[] = array( "title" => "社区满减金额", "field" => "shequdeductenough", "width" => 24 );
            $columns[] = array( "title" => "社区优惠券金额", "field" => "shequcouponprice", "width" => 24 );
    ...
    public function shequpay()
    {
        ...
        if( empty($handpay) && empty($item["applytype"]) ) 
        {
            ...
            $result = m("finance")->pay($shequ_user["payopenid"], 1, $payprice, $item["applyno"], "社区提现申请打款");
            if( is_error($result) ) 
            {
        ...
        pdo_update("ewei_shop_shequ_bill", $change_data, array( "id" => $id ));
        $orderids = iunserializer($item["passorderids"]);
        foreach( $orderids as $key => $orderid ) 
        {
            pdo_update("ewei_shop_order", array( "shequapply" => 3 ), array( "id" => $orderid ));
        }
        show_json(1);
    }
    ...
    public function confirm()
    {
        ...
        if( $type == 1 ) 
        {
            ...
            $pass_data = $this->model->getPassApplyCredit($item["shequid"], $bpid, $item["creditrate"], $item["isbillcredit"]);
            ...
            foreach( $orderids as $key => $orderid ) 
            {
                ...
                {
                    pdo_update("ewei_shop_creditshop_log", array( "shequapply" => 2 ), array( "id" => $orderid ));
                }
                else
                {
                    pdo_update("ewei_shop_creditshop_log", array( "shequapply" => -1 ), array( "id" => $orderid ));
                }
