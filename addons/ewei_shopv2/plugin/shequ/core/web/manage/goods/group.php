...
class Group_EweiShopV2Page extends shequWebPage
{
	...
	protected function post()
	{
		...
		if (!empty($id)) {
			$item = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_goods_group') . ' WHERE id=:id and uniacid=:uniacid and shequid=:shequid limit 1 ', array(':id' => $id, ':uniacid' => $_W['uniacid'], ':shequid' => $shequid));

			if (!empty($item['goodsids'])) {
				$item['goodsids'] = trim($item['goodsids'], ',');
				$goods = pdo_fetchall('select id,title,thumb from ' . tablename('ewei_shop_goods') . (' where id in (' . $item['goodsids'] . ') and status=1 and deleted=0 and uniacid=' . $_W['uniacid'] . ' and shequid=' . $shequid . ' order by instr(\'' . $item['goodsids'] . '\',id)'));
			}
		}
		...
		if ($_W['ispost']) {
			...
			$data = array('name' => $groupname, 'shequid' => $shequid, 'goodsids' => implode(',', $goodsids), 'enabled' => $enabled);
			...
			show_json(1, array('url' => shequUrl('goods/group/edit', array('id' => $id))));
		}
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,name FROM ' . tablename('ewei_shop_goods_group') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid'] . ' AND shequid=' . $shequid);
	...
	public function query()
	{
		...
		$condition = ' and enabled=1 and uniacid=:uniacid and shequid=:shequid';
