...
$shequ_user = $_W["shequ_user"];
$id = intval($_GPC["id"]);
$params1 = array( ":uniacid" => $_W["uniacid"], ":shequid" => $_W["shequid"] );
$not_add = 0;
$maxgoods = intval($shequ_user["maxgoods"]);
// 阿飞添加多个权限
...
$chun_levels = array_column($levels,'id');rsort($chun_levels); //2019-2-19阿飞添加默认分红金额为0.02
...
if( 0 < $maxgoods ) 
{
	$sql = "SELECT COUNT(1) FROM " . tablename("ewei_shop_goods") . " where uniacid=:uniacid and shequid=:shequid";
...
$item = pdo_fetch("SELECT * FROM " . tablename("ewei_shop_goods") . " WHERE id = :id and uniacid=:uniacid and shequid=:shequid", array( ":id" => $id, ":uniacid" => $_W["uniacid"], ":shequid" => $_W["shequid"] ));
...
$category = $this->model->getshequCategory();
//$levels = array( array( "id" => 0, "key" => "shequ", "levelname" => "促销价" ) );
...
if( $_W["ispost"] ) 
{
    ...
    $data["showlevels"] = (is_array($_GPC["showlevels"]) ? implode(",", $_GPC["showlevels"]) : "");
    $data["buylevels"] = (is_array($_GPC["buylevels"]) ? implode(",", $_GPC["buylevels"]) : "");
    $data["buycommissionlevels"] = (is_array($_GPC["buycommissionlevels"]) ? implode(",", $_GPC["buycommissionlevels"]) : ""); //阿飞添加分销等级购买
        

	if( p("commission") ) 
	{
		$cset = p("commission")->getSet();
		if( !empty($cset["level"]) && $shequ_user["commissionchecked"] == 1 ) 
		{
	...
	$data["bounsprice"] = $_GPC["bounsprice"];
	
	$data["costprice"] = $_GPC["costprice"];             //2019-2-21阿飞添加
	$data["settlementprice"] = $_GPC["settlementprice"]; //2019-2-21阿飞添加
	$data["neigou_url"] = trim($_GPC["neigou_url"]); 	 //2019-2-21阿飞添加
	...
	if( empty($_W["shequ_user"]["goodschecked"]) ) 
	{
		if( $needcheck ) 
		{
			$data["checked"] = 1;
	...
	if(empty($_GPC['bounsprice'])){ //2019-2-19阿飞添加默认分红金额为0.02
		$bounsprice = 0; 
		$min_price = array();
		if($item['hasoption']==1){
			$dis_counts_options_pre = pdo_fetchall("select marketprice from ".tablename('ewei_shop_goods_option')." where uniacid=".$_W['uniacid']." and goodsid=".$id);
			$min_price = min($dis_counts_options_pre);//多规格商品获取最低售价
		}else{
			$min_price['marketprice'] = $_GPC['marketprice'];//单规格商品获取售价
		}
		// if($discounts_json!='{"default":""}'){ 	//设置会员折扣 社区不存在编辑会员价格的权限所以按照售价结算注释掉此块
			// $dis_counts_pre = json_decode($discounts_json,true);
			// foreach($chun_levels as $val){
				// if(!empty($dis_counts_pre['level'.$val."_pay"])){
					// $bounsprice = $dis_counts_pre['level'.$val."_pay"] * 0.02;
					// break;
				// }
				// if(!empty($dis_counts_pre['level'.$val])){
					// $bounsprice = ($min_price['marketprice'] * $dis_counts_pre['level'.$val]) / 10;
					// break;
				// }
			// }
		// }
		if(empty($bounsprice)){ //如果所有等级都没有设置会员价
			$bounsprice = $min_price['marketprice'] * 0.02;
		}
		pdo_update("ewei_shop_goods",array('bounsprice'=>$bounsprice),array('id'=>$id));
	}
	show_json(1, array( "url" => shequUrl("goods/edit", array( "id" => $id, "tab" => str_replace("#tab_", "", $_GPC["tab"]) )) ));
}
if( !empty($id) ) 
{
	if( empty($item) ) 
	...
	{
		$stores = pdo_fetchall("select id,storename from " . tablename("ewei_shop_shequ_store") . " where id in (" . $item["storeids"] . " ) and uniacid=" . $_W["uniacid"] . " and shequid=" . $_W["shequid"]);
	}
	...
	{
		$item["buycommissionlevels"] = explode(",", $item["buycommissionlevels"]);
	}        
    // 阿飞添加分销等级购买 e
}
if( p("commission") ) 
...
$dispatch_data = pdo_fetchall("select * from " . tablename("ewei_shop_dispatch") . " where uniacid=:uniacid and shequid=:shequid and enabled=1 order by displayorder desc", array( ":uniacid" => $_W["uniacid"], ":shequid" => $_W["shequid"] ));
if( $com_virtual ) 
{
	$virtual_types = pdo_fetchall("select * from " . tablename("ewei_shop_virtual_type") . " where uniacid=:uniacid and shequid=:shequid order by id asc", array( ":uniacid" => $_W["uniacid"], ":shequid" => $_W["shequid"] ));
}
