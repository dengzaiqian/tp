...
class Index_EweiShopV2Page extends shequWebPage
{
	public function main($goodsfrom = '')
	{
		...
		$condition = ' WHERE g.`uniacid` = :uniacid and g.`shequid`=:shequid';
		...
		if (!empty($total)) {
			$sql = 'SELECT g.* FROM ' . tablename('ewei_shop_goods') . 'g' . $sqlcondition . $condition . $groupcondition . " ORDER BY g.`status` DESC, g.`shequdisplayorder` DESC,\r\n                g.`id` DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
		...
		$categorys = $this->getCategory($category_set['level'], $_W['shequid']);
		// print_r($categorys);
		// print_r($category);
	...
	public function getCategory($level, $shequid = 0)
	{
		global $_W;
		global $_GPC;
		$fullname = true;
		...
		$shequ_category = $this->model->getSet('shequ_category', $shequid);
		foreach($shequ_category as $key => $m_cate){
			if($m_cate==1){
				$mer_category[] = $key;
			}
		}
		// print_r($mer_category);
		$allcategory=array();
	...
	public function change()
	{
		...
		if (!in_array($type, array('title', 'marketprice', 'total', 'goodssn', 'productsn', 'displayorder', 'shequdisplayorder'))) {
			show_json(0, array('message' => '参数错误'));
		}
	...
	public function query()
	{
		...
		$condition = ' and status=1 and deleted=0 and uniacid=:uniacid and shequid=:shequid';
	...
	public function goods_selector()
	{
		...
		if ((int) $_GPC['shequid']) {
			$condition .= ' and shequid = ' . (int) $_W['shequid'];
		}
		...
		if (empty($list) && $page !== 1) {
			header('location:' . webUrl('goods.goods_selector', array('shequid' => $_W['shequid'])));
			exit();
		}
		else {
