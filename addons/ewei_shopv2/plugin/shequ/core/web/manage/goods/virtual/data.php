...
class Data_EweiShopV2Page extends shequWebPage
{
	...
	public function main()
	{
		...
		if (empty($typeid)) {
			$this->message('Url参数错误！请重试！', shequUrl('goods/virtual/temp'), 'error');
			exit();
		}
		...
		$psize = 60;
		$type = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_virtual_type') . ' WHERE id=:id and uniacid=:uniacid and shequid=:shequid ', array(':id' => $typeid, ':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']));
		$type['fields'] = iunserializer($type['fields']);
		$condition = ' and d.typeid=:typeid and d.uniacid=:uniacid and d.shequid=:shequid';
		$params = array(':typeid' => $typeid, ':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']);
	...
	protected function post()
	{
		...
		$item = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_virtual_type') . ' WHERE id=:id and uniacid=:uniacid and shequid=:shequid ', array(':id' => $_GPC['typeid'], ':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']));
		...
		if (!empty($editid)) {
			$data = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_virtual_data') . ' WHERE id=:id and typeid=:typeid and uniacid=:uniacid and shequid=:shequid ', array(':id' => $editid, ':typeid' => $typeid, ':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']));
			$data['edit'] = $editid;
		}
		...
		if ($_W['ispost']) {
			...
			if (!empty($typeid)) {
				$item = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_virtual_type') . ' WHERE id=:id and uniacid=:uniacid and shequid=:shequid ', array(':id' => $typeid, ':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']));
				$item['fields'] = iunserializer($item['fields']);
				...
				if (!empty($item['fields'])) {
					...
					foreach ($tpids as $index => $id) {
						...
						$insert = array('typeid' => $_GPC['typeid'], 'pvalue' => $values['key'], 'fields' => iserializer($values), 'uniacid' => $_W['uniacid'], 'shequid' => $_W['shequid']);
						$datas = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_virtual_data') . ' WHERE id=:id and typeid=:typeid and uniacid=:uniacid and shequid=:shequid ', array(':id' => $id, ':typeid' => $typeid, ':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']));

						if (empty($datas)) {
							$keydata = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_virtual_data') . ' WHERE pvalue=:pvalue  and typeid=:typeid and uniacid=:uniacid and shequid=:shequid', array(':pvalue' => $insert['pvalue'], ':typeid' => $typeid, ':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']));
							...
							$keydata = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_virtual_data') . ' WHERE pvalue=:pvalue and id<>:id and typeid=:typeid and uniacid=:uniacid and shequid=:shequid', array(':pvalue' => $insert['pvalue'], ':id' => $id, ':typeid' => $typeid, ':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']));
					...
					if (!empty($noinsert)) {
						...
						show_json(1, array('message' => '部分数据保存成功！' . $tip, 'url' => shequUrl('virtual/data', array('typeid' => $typeid))));
					}
					else {
						show_json(1, array('url' => shequUrl('goods/virtual/data', array('typeid' => $typeid))));
					}
				}
			}
		}
	...
	public function delete()
	{
		...
		$types = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_virtual_data') . (' WHERE id in( ' . $id . ' ) AND typeid=' . $typeid . ' and  uniacid=') . $_W['uniacid'] . ' and  shequid=' . $_W['shequid']);
	...
	public function export()
	{
		...
		global $_GPC;
		$typeid = intval($_GPC['typeid']);
		$type = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_virtual_type') . ' WHERE id=:id and uniacid=:uniacid and shequid=:shequid limit 1 ', array(':id' => $typeid, ':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']));
		...
		$list = pdo_fetchall('SELECT d.*,o.carrier,m.avatar,m.nickname FROM ' . tablename('ewei_shop_virtual_data') . ' d ' . ' left join ' . tablename('ewei_shop_member') . ' m on m.openid = d.openid and m.uniacid = d.uniacid and d.shequid=0' . ' left join ' . tablename('ewei_shop_order') . ' o on o.id = d.orderid ' . (' where  1 ' . $condition . ' order by usetime desc'), $params);
	...
	public function temp()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_virtual_type') . ' WHERE id=:id and uniacid=:uniacid and shequid=:shequid ', array(':id' => $id, ':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']));
	...
	public function import()
	{
		...
		foreach ($datas as $d) {
			if ($d['pvalue'] != '') {
				if (!empty($d['status']) && $d['status'] == 'del') {
					$olddata = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_virtual_data') . ' WHERE pvalue=:pvalue and typeid=:typeid and uniacid=:uniacid and shequid=' . $_W['shequid'], array(':pvalue' => $d['pvalue'], ':typeid' => $_GPC['typeid'], ':uniacid' => $_W['uniacid']));
		...
		if ($status[0] == 1) {
			...
			if (!empty($noinsert)) {
				...
				$this->message('删除成功！', shequUrl('goods/virtual/data', array('typeid' => $_GPC['typeid'])));
			}
		}
		else {
			if ($status[0] == 2) {
				...
				if (!empty($noinsert)) {
					...
					$this->message('导入成功！', shequUrl('goods/virtual/data', array('typeid' => $_GPC['typeid'])));
				}
			}
		}
	}
