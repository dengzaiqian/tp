...
class Index_EweiShopV2Page extends shequWebPage
{
	public function main()
	{
		...
		$condition = ' and uniacid=:uniacid and shequid='.$_W['shequid'];
	...
	protected function post()
	{
		...
		$item = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_poster') . ' WHERE shequid='.$_W['shequid'].' and id =:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			if ($data['isdefault'] == 1) {
				pdo_update(
                    'ewei_shop_poster',
                    array('isdefault' => 0),
                    array('uniacid' => $_W['uniacid'], 'isdefault' => 1, 'type' => $data['type'],'shequid'=>$_W['shequid']));
			}
			...
			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where shequid='.$_W['shequid'].' and uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'ewei_shopv2', ':name' => 'ewei_shopv2:poster:' . $data['type']));
			...
			$ruleauto = pdo_fetch('select * from ' . tablename('rule') . ' where shequid='.$_W['shequid'].' and  uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'ewei_shopv2', ':name' => 'ewei_shopv2:poster:auto'));

			if (empty($ruleauto)) {
				$rule_data = array( 'shequid'=>$_W['shequid'],'uniacid' => $_W['uniacid'], 'name' => 'ewei_shopv2:poster:auto', 'module' => 'ewei_shopv2', 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule', $rule_data);
				$rid = pdo_insertid();
				$keyword_data = array( 'shequid'=>$_W['shequid'],'uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'ewei_shopv2', 'content' => 'EWEI_SHOPV2_POSTER', 'type' => 1, 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule_keyword', $keyword_data);
			}
		...
		if (mcv('coupon')) {
			if (!empty($item['subcouponid'])) {
				$subcoupon = mcv('coupon')->getCoupon($item['subcouponid']);
			}

			if (!empty($item['reccouponid'])) {
				$reccoupon = mcv('coupon')->getCoupon($item['reccouponid']);
			}
	...
	public function delete()
	{
		...
		$posters = pdo_fetchall('SELECT id,title,keyword,keyword2 FROM ' . tablename('ewei_shop_poster') . (' WHERE shequid='.$_W['shequid'].' and id in ( ' . $id . ' ) and uniacid=') . $_W['uniacid']);
		...
		foreach ($posters as $poster) {
			...
			$rule = pdo_fetchall('SELECT id,rid FROM ' . tablename('rule_keyword') . (' WHERE  shequid='.$_W['shequid'].' and  uniacid=:uniacid AND content IN (\'' . $poster['keyword'] . '\',\'' . $poster['keyword2'] . '\')'), array(':uniacid' => $_W['uniacid']), 'rid');
			$rule = array_keys($rule);
			m('common')->delrule($rule);
			pdo_delete('ewei_shop_poster', array('shequid'=>$_W['shequid'],'id' => $poster['id'], 'uniacid' => $_W['uniacid'],'shequid'=>$_W['shequid']));
			pdo_delete('ewei_shop_poster_log', array('shequid'=>$_W['shequid'],'posterid' => $poster['id'], 'uniacid' => $_W['uniacid'],'shequid'=>$_W['shequid']));
	...
	public function setdefault()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$poster = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_poster') . (' WHERE shequid='.$_W['shequid'].' and id = \'' . $id . '\''));

		if (empty($poster)) {
			show_json(0, '抱歉，海报不存在或是已经被删除！');
		}

		pdo_update('ewei_shop_poster', array('isdefault' => 0), array('uniacid' => $_W['uniacid'], 'isdefault' => 1, 'type' => $poster['type'],'shequid'=>$_W['shequid']));
		pdo_update('ewei_shop_poster', array('isdefault' => 1), array('uniacid' => $_W['uniacid'], 'id' => $poster['id'],'shequid'=>$_W['shequid']));
