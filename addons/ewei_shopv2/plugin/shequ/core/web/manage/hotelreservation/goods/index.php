...
class Index_EweiShopV2Page extends shequWebPage
{
	public function main()
	{
		global $_W;
		global $_GPC;
		$shequ_plugin = p('shequ');
		$shequ_data = m('common')->getPluginset('shequ');
		if ($shequ_plugin && $shequ_data['is_openshequ']) {
			$is_openshequ = 1;
		}
		else {
			$is_openshequ = 0;
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			show_json(1, array('url' => shequUrl('hotelreservation/goods/edit', array('id' => $id, 'tab' => str_replace('#tab_', '', $_GPC['tab'])))));
		}
