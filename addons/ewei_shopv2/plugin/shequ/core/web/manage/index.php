...
class Index_EweiShopV2Page extends shequWebPage
{
	...
	public function quit()
	{
		global $_W;
		global $_GPC;
		isetcookie('__shequ_' . $_W['uniacid'] . '_session', 0);
		isetcookie('__uniacid', 0);
		isetcookie('__acid', 0);
		unset($_SESSION['__shequ_uniacid']);
		header('location: ' . shequUrl('login') . '&i=' . $_W['uniacid']);
		exit();
	}
	...
	public function updatepassword()
	{
		...
		if ($_W['ispost']) {
			...
			$item = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_shequ_account') . ' WHERE id=:id AND uniacid=:uniacid AND shequid=:shequid', array(':id' => $_W['uniaccount']['id'], ':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']));
			...
			pdo_update('ewei_shop_shequ_account', $date, array('id' => $_W['uniaccount']['id'], 'uniacid' => $_W['uniacid'], 'shequid' => $_W['shequid']));
			show_json(1);
		}
	...
	public function search()
	{
		...
		$keyword = trim($_GPC['keyword']);
		$list = array();
		$history = $_GPC['shequ_history_search'];
	...
	'items' => array()
			...
			isetcookie('shequ_history_search', json_encode($history_new), 7 * 86400);
			$history = $history_new;
		}
	...
	public function clearhistory()
	{
		...
		if ($_W['ispost']) {
			...
			if (empty($type)) {
				isetcookie('shequ_history_url', '', -7 * 86400);
			}
			else {
				isetcookie('shequ_history_search', '', -7 * 86400);
			}
		}
	...
	public function switchversion()
	{
		...
		$id = intval($_GPC['id']);
		$set = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_version') . ' WHERE uid=:uid AND `type`=1', array(':uid' => $_W['shequuid']));
		$data = array('type' => 1, 'version' => !empty($_W['shopversion']) ? 0 : 1);

		if (empty($set)) {
			$data['uid'] = $_W['shequuid'];
