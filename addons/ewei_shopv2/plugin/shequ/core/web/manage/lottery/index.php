...

{
    public function main()
    {
        global $_W;
        global $_GPC;
        $this->model->CheckPlugin('lottery');
        $pindex = max(1, intval($_GPC["page"]));
        $psize = 10;
        $params = array( ":uniacid" => $_W["uniacid"] );
        $condition = " and uniacid=:uniacid and `is_delete`=0 and `shequid`=".$_W['shequid'];
    ...
    protected function post()
    {
        ...
        if( $id ) 
        {
            $item = pdo_fetch("SELECT * FROM " . tablename("ewei_shop_lottery") . " WHERE shequid=".$_W['shequid']." and lottery_id =:lottery_id and uniacid=:uniacid limit 1", array( ":lottery_id" => $id, ":uniacid" => $_W["uniacid"] ));
    ...
    public function delete()
    {
        ...
        $posters = pdo_fetchall("SELECT lottery_id,lottery_title FROM " . tablename("ewei_shop_lottery") . " WHERE shequid=".$_W['shequid']." and lottery_id in ( " . $id . " ) and uniacid=" . $_W["uniacid"]);
    ...
    {
        global $_W;
        $sql = "SELECT COUNT(1) FROM " . tablename("ewei_shop_lottery") . " WHERE shequid=".$_W['shequid']." and uniacid = :uniacid AND task_type = :task_type AND is_delete = 0";
        echo $sql."<br>";
        $res = pdo_fetchcolumn($sql, array( ":uniacid" => $_W["uniacid"], ":task_type" => $task_type ));
        $res = intval($res);
        if( $res === 1 ) 
        {
            $sql = "SELECT COUNT(1) FROM " . tablename("ewei_shop_lottery") . " WHERE  shequid=".$_W['shequid']." and uniacid = :uniacid AND task_type = :task_type AND lottery_id = :id AND is_delete = 0";
            echo $sql."<br>";
