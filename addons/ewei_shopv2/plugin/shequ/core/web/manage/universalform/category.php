...
class Category_EweiShopV2Page extends shequWebPage
{
	public function main()
	{
		...
		if (!empty($_GPC['catname'])) {
			...
			foreach ($_GPC['catname'] as $id => $catname ) {
				...
				if ($id == 'new') {
					pdo_insert('ewei_shop_universalform_category', array('name' => $catname,'shequid'=>$_W['shequid'],'uniacid' => $_W['uniacid']));
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_universalform_category') . ' WHERE shequid='.$_W['shequid'].' and uniacid = \'' . $_W['uniacid'] . '\' ORDER BY id DESC');
	...
	public function delete()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT id,name FROM ' . tablename('ewei_shop_universalform_category') . ' WHERE shequid='.$_W['shequid'].' and  id = \'' . $id . '\' AND uniacid=' . $_W['uniacid'] . '');
