...
class Credit_EweiShopV2Page extends shequWebPage
{
    protected function applyData($status, $st)
    {
        global $_W;
        global $_GPC;
        empty($status) and $shequid = $_W["shequid"];
        ...
        $psize = 20;
        $condition = " and b.uniacid=:uniacid and b.status=:status and b.shequid=:shequid and b.creditstatus = 1";
        $params = array( ":uniacid" => $_W["uniacid"], ":status" => $status, ":shequid" => $shequid );
        ...
        $sql = "select b.*,u.shequname,u.realname,u.mobile,u.iscreditmoney,b.creditrate,b.isbillcredit from " . tablename("ewei_shop_shequ_bill") . " b " . " left join " . tablename("ewei_shop_shequ_user") . " u on b.shequid = u.id" . " where 1 " . $condition . " ORDER BY " . $orderby . " desc ";
        ...
        $total = pdo_fetchcolumn("select count(b.id) from" . tablename("ewei_shop_shequ_bill") . " b " . " left join " . tablename("ewei_shop_shequ_user") . " u on b.shequid = u.id" . " where 1 " . $condition, $params);
    ...
    protected function post()
    {
        ...
        global $_GPC;
        $shequid = $_W["shequid"];
        $item = $this->model->getshequCredit($shequid, 1);
        $list = $this->model->getshequCreditList($shequid, "", 0, $item["creditrate"], $item["iscreditmoney"]);
        $order_num = count($list);
        $set = m("common")->getPluginset("shequ");
        if( empty($set) ) 
        {
            $set = $this->model->getPluginsetByshequ("shequ");
        }

        $last_data = $this->getLastApply($shequid);
        $type_array = array(  );
        ...
        if( $set["applycashalipay"] == 1 ) 
        {
            ...
            {
                $type_last = $this->getLastApply($shequid, 2);
                if( !empty($type_last) ) 
                {
        ...
        if( $set["applycashcard"] == 1 ) 
        {
            ...
            {
                $type_last = $this->getLastApply($shequid, 3);
                if( !empty($type_last) ) 
                {
        ...
        if( $_W["ispost"] ) 
        {
            ...
            $insert["shequid"] = $shequid;
            $insert["applyno"] = m("common")->createNO("shequ_bill", "applyno", "MO");
            ...
            pdo_insert("ewei_shop_shequ_bill", $insert);
            $billid = pdo_insertid();
            foreach( $list as $k => $v ) 
            {
                $orderid = $v["id"];
                ...
                pdo_insert("ewei_shop_shequ_billo", $insert_data);
                $change_order_data = array(  );
                $change_order_data["shequapply"] = 1;
            ...
            $this->model->sendMessage(array( "shequname" => $shequ_user["shequname"], "money" => $insert["realprice"], "realname" => $shequ_user["realname"], "mobile" => $shequ_user["mobile"], "applytime" => time() ), "shequ_apply_money");
    ...
    public function ajaxgettotals()
    {
        global $_W;
        $shequid = $_W["shequid"];
        $totals = $this->model->getshequApplyTotals($shequid);
        $result = (empty($totals) ? array(  ) : $totals);
        show_json(1, $result);
    }

    public function getLastApply($shequid, $applytype = -1)
    {
        global $_W;
        $params = array( ":uniacid" => $_W["uniacid"], ":shequid" => $shequid );
        $sql = "select applytype,alipay,bankname,bankcard,applyrealname from " . tablename("ewei_shop_shequ_bill") . " where shequid=:shequid and uniacid=:uniacid";
        if( -1 < $applytype ) 
        {
