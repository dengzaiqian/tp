...
class Index_EweiShopV2Page extends shequWebPage
{
	public function main()
	{
		...
		$condition .= ' and (shequ=' . intval($_W['shequid']) . ' or uniacid=0)';
		...
		$category = pdo_fetchall('SELECT id, name FROM ' . tablename('ewei_shop_diypage_template_category') . ' WHERE uniacid=:uniacid and shequ=:shequ order by id desc ', array(':shequ' => intval($_W['shequid']), ':uniacid' => $_W['uniacid']));
	...
	public function delete()
	{
		...
		if ($_W['ispost']) {
			...
			$item = pdo_fetch('SELECT id, name, uniacid FROM ' . tablename('ewei_shop_diypage_template') . ' WHERE id=:id and (uniacid=:uniacid or uniacid=0) and (shequ=:shequ or uniacid=0) ', array(':shequ' => intval($_W['shequid']), ':uniacid' => $_W['uniacid'], ':id' => $tid));
			...
			if (!empty($item)) {
				if (empty($item['uniacid'])) {
					...
					pdo_update('ewei_shop_diypage_template', array('deleted' => 1), array('id' => $tid, 'shequ' => intval($_W['shequid'])));
				}
				else {
