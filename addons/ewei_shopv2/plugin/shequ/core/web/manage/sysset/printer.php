...
class Printer_EweiShopV2Page extends shequWebPage
{
	...
	protected function post()
	{
		...
		if (!empty($id)) {
			$list = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_member_printer_template') . ' WHERE id=:id and uniacid=:uniacid and shequid=:shequid', array(':id' => $id, ':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']));
		...
		if ($_W['ispost']) {
			$data = array('uniacid' => $_W['uniacid'], 'shequid' => $_W['shequid'], 'type' => intval($_GPC['type']), 'title' => trim($_GPC['title']), 'print_title' => trim($_GPC['print_title']), 'print_style' => trim($_GPC['print_style']), 'code' => intval($_GPC['code']), 'qrcode' => trim($_GPC['qrcode']));
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('ewei_shop_member_printer_template') . (' WHERE id in( ' . $id . ' ) AND uniacid=:uniacid AND shequid=:shequid'), array(':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']));
	...
	protected function printer_post()
	{
		...
		if (!empty($id)) {
			$list = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_member_printer') . ' WHERE id=:id and uniacid=:uniacid and shequid=:shequid', array(':id' => $id, ':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']));
			$print_data = json_decode($list['print_data'], true);
		...
		if ($_W['ispost']) {
			$data = array('uniacid' => $_W['uniacid'], 'shequid' => $_W['shequid'], 'type' => intval($_GPC['type']), 'title' => trim($_GPC['title']));
			...
			if ($data['type'] == 6) {
				...
				$res = com_run('printer::bindYilianyunPrinter', $client_id, $client_secret, $machine_code, $msign, $data['shequid']);
	...
	public function printer_delete()
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('ewei_shop_member_printer') . (' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid'] . ' AND shequid=' . $_W['shequid']));
	...
	public function set()
	{
		...
		$data = com_run('printer::getPrinterSet', $_W['shequid']);
		$order_printer_array = $data['order_printer'];
		$ordertype = $data['ordertype'];
		$order_template = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_member_printer_template') . ' WHERE uniacid=:uniacid AND shequid=:shequid', array(':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']));
		include $this->template();
	}
}
