...
class Member_increase_EweiShopV2Page extends shequWebPage
{
	public function main()
	{
		...
		if (!empty($days)) {
			...
			while (0 <= $i) {
				...
				$condition = ' and uniacid=:uniacid and ' . $timefield . '>=:starttime and ' . $timefield . '<=:endtime and  openid in ( SELECT distinct openid from ' . tablename('ewei_shop_order') . '   WHERE uniacid =:uniacid and shequid=:shequid )';
				$params = array(':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid'], ':starttime' => strtotime($time . ' 00:00:00'), ':endtime' => strtotime($time . ' 23:59:59'));
			...
			if (!empty($year) && !empty($month)) {
				...
				while ($d <= $lastday) {
					$condition = ' and uniacid=:uniacid and ' . $timefield . '>=:starttime and ' . $timefield . '<=:endtime and  openid in ( SELECT distinct openid from ' . tablename('ewei_shop_order') . '   WHERE uniacid =:uniacid and shequid=:shequid )';
					$params = array(':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid'], ':starttime' => strtotime($year . '-' . $month . '-' . $d . ' 00:00:00'), ':endtime' => strtotime($year . '-' . $month . '-' . $d . ' 23:59:59'));
				...
				if (!empty($year)) {
					...
					foreach ($months as $m) {
						...
						$condition = ' and uniacid=:uniacid and ' . $timefield . '>=:starttime and ' . $timefield . '<=:endtime and  openid in ( SELECT distinct openid from ' . tablename('ewei_shop_order') . '   WHERE uniacid =:uniacid and shequid=:shequid )';
						$params = array(':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid'], ':starttime' => strtotime($year . '-' . $m['data'] . '-01 00:00:00'), ':endtime' => strtotime($year . '-' . $m['data'] . '-' . $lastday . ' 23:59:59'));
