...
class Taobaocsv_EweiShopV2Page extends shequWebPage
{
	public function main()
	{
		...
		if ($_W['ispost']) {
			...
			m('cache')->set('taobaoCSV', $items, $_W['uniacid'] . $_W['shequid']);
			$uploadStart = '1';
			$uploadnum = $num;
		}
	...
	public function fetch()
	{
		...
		if (empty($items)) {
			$items = m('cache')->get('taobaoCSV', $_W['uniacid'] . $_W['shequid']);
		}

		$taobao_plugin = p('taobao');
		$ret = $taobao_plugin->save_taobaocsv_goods($items[$num], $shequid);
