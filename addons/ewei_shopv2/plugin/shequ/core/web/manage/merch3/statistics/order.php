...
class Order_EweiShopV2Page extends shequWebPage
{
	public function main()
	{
		...
		$psize = 20;
		$condition = ' and (u.id='.$_W['shequid'].' or u.parentshequid='.$_W['shequid'].') and o.uniacid=:uniacid and o.shequid>0 and o.status>=1';
		...
		if (!empty($searchfield) && !empty($_GPC['keyword'])) {
			if ($searchfield == 'ordersn') {
				...
				if ($searchfield == 'shequname') {
					$condition .= ' and u.shequname like :keyword';
				}
			}
		...
		$sql = 'select o.*, a.realname as addressname,m.realname,u.shequname from ' . tablename('ewei_shop_order') . ' o ' . ' left join ' . tablename('ewei_shop_member') . ' m on o.openid = m.openid ' . ' left join ' . tablename('ewei_shop_member_address') . ' a on a.id = o.addressid ' . ' left join ' . tablename('ewei_shop_shequ_user') . ' u on u.id = o.shequid ' . (' where 1 ' . $condition . ' ');
		...
		unset($row);
		$total = pdo_fetchall('select o.id from ' . tablename('ewei_shop_order') . ' o ' . ' left join ' . tablename('ewei_shop_member') . ' m on o.openid = m.openid ' . ' left join ' . tablename('ewei_shop_member_address') . ' a on a.id = o.addressid ' . ' left join ' . tablename('ewei_shop_shequ_user') . ' u on u.id = o.shequid ' . (' where 1 ' . $condition), $params);
