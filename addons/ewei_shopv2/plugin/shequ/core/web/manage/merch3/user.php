...
class User_EweiShopV2Page extends shequWebPage
{
    public function main()
    {
        ...
        $sql = "select  u.*,g.groupname  from " . tablename("ewei_shop_shequ_user") . "  u " . " left join  " . tablename("ewei_shop_shequ_group") . " g on u.groupid = g.id " . " where u.uniacid=:uniacid " . $condition . " ORDER BY " . $sortfield . " desc";
    ...
    protected function post()
    {
        ...
        if( empty($id) ) 
        {
            $max_flag = p('shequ')->checkMaxshequUser(1);
            if( $max_flag == 1 ) 
            {
        ...
        $item = pdo_fetch("select * from " . tablename("ewei_shop_shequ_user") . " where parentshequid=".$_W['shequid']." and id=:id and uniacid=:uniacid limit 1", array( ":id" => $id, ":uniacid" => $_W["uniacid"] ));
        if( empty($item) ) 
        {
            $item["iscredit"] = 1;
        ...
        if( $_W["ispost"] ) 
        {
            ...
            $usercount = pdo_fetchcolumn("select count(*) from " . tablename("ewei_shop_shequ_account") . " where parentshequid=". $_W['shequid'] . $where . " limit 1", $params);
            if( 0 < $usercount ) 
            {
                show_json(0, "账户名 " . $username . " 已经存在!");
            }
            ...
            if( empty($account) || empty($account["salt"]) || !empty($_GPC["pwd"]) ) 
            {
                ...
                while( 1 ) 
                {
                    $saltcount = pdo_fetchcolumn("select count(*) from " . tablename("ewei_shop_shequ_account") . " where parentshequid=". $_W['shequid'] ." and salt=:salt limit 1", array( ":salt" => $salt ));
                    if( $saltcount <= 0 ) 
                    {
                        break;
                    }
        ...
        $plugins_data = p('shequ')->getPluginList();
        $plugins_list = $plugins_data["plugins_list"];     
        
        $groups = p('shequ')->getGroups();
        $category = p('shequ')->getCategory();
        include($this->template());
    }
    ...
    public function get_show_money()
    {
        ...
        if( !empty($id) ) 
        {
            $tmoney = p('shequ')->getshequOrderTotalPrice($id);
