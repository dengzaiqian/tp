...
class Comment_EweiShopV2Page extends shequWebPage
{
	...
	protected function getList($checked = 0)
	{
		...
		$psize = 20;
		$condition = ' and c.uniacid=:uniacid and c.deleted=0 and g.shequid=' . $_W['shequid'];
	...
	public function delete()
	{
		...
		foreach ($items as $item) {
			pdo_update('ewei_shop_creditshop_comment', array('deleted' => 1), array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'shequid' => $_W['shequid']));
			$goods = pdo_fetch('select id,thumb,title from ' . tablename('ewei_shop_creditshop_goods') . ' where id=:id and shequid = ' . $_W['shequid'] . ' and uniacid=:uniacid limit 1', array(':id' => $item['goodsid'], ':uniacid' => $_W['uniacid']));
