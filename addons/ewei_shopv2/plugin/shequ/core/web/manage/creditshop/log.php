...
class Log_EweiShopV2Page extends shequWebPage
{
	...
	protected function getData($type)
	{
		...
		$psize = 20;
		$condition = ' and log.uniacid=:uniacid and g.shequid = ' . $_W['shequid'] . ' ';
		...
		foreach ($list as &$row) {
			...
			if (!empty($row['addressid'])) {
				...
				if (0 < intval($row['gshequid'])) {
					$stores = pdo_fetch('select * from ' . tablename('ewei_shop_shequ_store') . ' where id=:id and uniacid=:uniacid and shequid=:shequid and status=1 and type in(2,3)', array(':id' => $row['storeid'], ':uniacid' => $_W['uniacid'], ':shequid' => $row['gshequid']));
				}
				else {
	...
	public function detail()
	{
		...
		$id = intval($_GPC['id']);
		$log = pdo_fetch('select * from ' . tablename('ewei_shop_creditshop_log') . ' where id=:id and uniacid=:uniacid and shequid = :shequid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']));
		...
		if ($goods['isverify']) {
			if (!empty($log['storeid'])) {
				if (0 < $shequid) {
					$stores = pdo_fetch('select * from ' . tablename('ewei_shop_shequ_store') . ' where id=:id and uniacid=:uniacid and shequid=:shequid and status=1 and type in(2,3)', array(':id' => $log['storeid'], ':uniacid' => $_W['uniacid'], ':shequid' => $shequid));
				}
				else {
