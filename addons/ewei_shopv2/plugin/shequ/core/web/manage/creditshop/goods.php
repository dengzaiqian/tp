...
class Goods_EweiShopV2Page extends shequWebPage 
{
	public function main() 
	{
		...
		$psize = 20;
		$condition = " uniacid = :uniacid AND deleted = :deleted and shequid = " . $_W["shequid"];
	...
	protected function post() 
	{
		...
		$item = pdo_fetch("SELECT * FROM " . tablename("ewei_shop_creditshop_goods") . " WHERE id =:id and shequid = " . $shequid . " and uniacid=:uniacid limit 1", array( ":uniacid" => $_W["uniacid"], ":id" => $id ));
		if( !empty($item) ) 
		{
		...
		if( !empty($item["storeids"]) ) 
		{
			if( 0 < $shequid ) 
			{
				$stores = pdo_fetchall("select * from " . tablename("ewei_shop_shequ_store") . " where id in (" . $item["storeids"] . ") and uniacid=:uniacid and shequid=:shequid and status=1 and type in(2,3)", array( ":uniacid" => $_W["uniacid"], ":shequid" => $shequid ));
			}
			else 
			{
	...
	public function query() 
	{
		...
		$condition = " and status=1 and deleted=0 and uniacid=:uniacid and type = 1 and groupstype = 0 \r\n                    and isdiscount = 0 and istime = 0 and ifnull(bargain,0)=0 and ispresell = 0 and shequid = " . $shequid . " ";
		if( !empty($kwd) ) 
		{
