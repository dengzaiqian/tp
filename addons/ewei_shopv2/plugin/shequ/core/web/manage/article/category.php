...
class Category_EweiShopV2Page extends shequWebPage
{
	public function main()
	{
		global $_W;
		global $_GPC;
		$list = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_article_category') . (' WHERE shequid='.$_W["shequid"].' and uniacid = \'' . $_W['uniacid'] . '\' ORDER BY displayorder desc ,id desc'));
	...
	public function save()
	{
		...
		if (!empty($_GPC['cate_new'])) {
			foreach ($_GPC['cate_new'] as $cate_new) {
				...
				pdo_insert('ewei_shop_article_category', array('shequid'=>$_W['shequid'],'category_name' => $cate_new, 'uniacid' => $_W['uniacid']));
	...
	public function delete()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT id,category_name FROM ' . tablename('ewei_shop_article_category') . (' WHERE shequid='.$_W["shequid"].' and  id = \'' . $id . '\' AND uniacid=') . $_W['uniacid'] . '');
