...
class Set_EweiShopV2Page extends shequWebPage
{
	public function main()
	{
		...
		$shequ_key = "shequ_article_".$_W['shequid'];
		
		$article_sys = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_article_sys') . ' WHERE shequid=:shequid and uniacid=:uniacid limit 1 ', array(':shequid'=>$_W['shequid'],':uniacid' => $_W['uniacid']));
		$set = m('common')->getPluginset($shequ_key);
		...
		if ($_W['ispost']) {
			...
			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where shequid=:shequid and uniacid=:uniacid and module=:module and name=:name limit 1', array('shequid'=>$_W['shequid'],':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => 'ewei_shopv2文章营销入口设置'));
			...
			$rule_data = array('shequid'=>$_W['shequid'],'uniacid' => $_W['uniacid'], 'name' => 'ewei_shopv2文章营销入口设置', 'module' => 'cover', 'displayorder' => 0, 'status' => 1);
			...
			if (empty($rule)) {
				...
				pdo_update('rule', $rule_data, array('shequid'=>$_W['shequid'],'id' => $rule['id']));
				$rid = $rule['id'];
			}

			$keyword_data = array('shequid'=>$_W['shequid'],'uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'cover', 'content' => trim($article_keyword), 'type' => 1, 'displayorder' => 0, 'status' => 1);

			if (empty($keyword)) {
				pdo_insert('rule_keyword', $keyword_data);
			}
			else {
				pdo_update('rule_keyword', $keyword_data, array('shequid'=>$_W['shequid'],'id' => $keyword['id']));
			}

			$cover_data = array('shequid'=>$_W['shequid'],'uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => $this->modulename, 'title' => trim($article_title), 'description' => '', 'thumb' => $article_image, 'url' => mobileUrl('article/list'));

			if (empty($cover)) {
				pdo_insert('cover_reply', $cover_data);
			}
			else {
				pdo_update('cover_reply', $cover_data, array('shequid'=>$_W['shequid'],'id' => $cover['id']));
			}

			$sys = pdo_fetch('select * from ' . tablename('ewei_shop_article_sys') . ' where shequid=:shequid and uniacid=:uniacid limit 1', array(':shequid'=>$_W['shequid'],':uniacid' => $_W['uniacid']));
			// $sys = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_article_sys') . ' WHERE shequid=:shequid and uniacid=:uniacid limit 1 ', array(':shequid'=>$_W['shequid'],':uniacid' => $_W['uniacid']));
			...
			if (empty($sys)) {
				...
				pdo_update('ewei_shop_article_sys', $arr, array('shequid'=>$_W['shequid'],'uniacid' => $_W['uniacid']));
			}
		...
		$url = mobileUrl('article/list', array('shequid'=>$_W['shequid']), true);
