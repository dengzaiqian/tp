...
class Index_EweiShopV2Page extends shequWebPage 
{
	public function main() 
	{
		...
		$psize = 20;
		$articles = array( );
		$articles = pdo_fetchall("SELECT a.article_shequid,a.id,a.displayorder, a.article_title,a.article_category,a.article_keyword2,a.article_date,a.article_readnum,a.article_likenum,a.article_state,c.category_name FROM " . tablename("ewei_shop_article") . " a left join " . tablename("ewei_shop_article_category") . " c on c.id=a.article_category  WHERE a.article_shequid=".$_W['shequid']." and  a.uniacid= :uniacid " . $select_title . $select_category . " order by displayorder desc,article_date desc LIMIT " . ($pindex - 1) * $psize . "," . $psize, array( ":uniacid" => $_W["uniacid"] ));
		$total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename("ewei_shop_article") . " a left join " . tablename("ewei_shop_article_category") . " c on c.id=a.article_category  WHERE a.article_shequid=".$_W['shequid']." and a.uniacid= :uniacid " . $select_title . $select_category, array( ":uniacid" => $_W["uniacid"] ));
		$pager = pagination2($total, $pindex, $psize);
		if( !empty($articles) ) 
		{
			foreach( $articles as $key => &$value ) 
			{
				if($value['article_shequid']){
					$url = mobileUrl("article", array("shequid"=>$value["article_shequid"], "aid" => $value["id"] ), true);
				}else{
		...
		$articlenum = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename("ewei_shop_article") . " WHERE article_shequid=".$_W['shequid']." and  uniacid= :uniacid ", array( ":uniacid" => $_W["uniacid"] ));
		$categorys = pdo_fetchall("SELECT * FROM " . tablename("ewei_shop_article_category") . " WHERE shequid=".$_W['shequid']." and  uniacid=:uniacid ", array( ":uniacid" => $_W["uniacid"] ));
	...
	protected function post() 
	{
		...
		if( $_W["ispost"] ) 
		{
			...
			$data["article_shequid"]=$_W['shequid']; 
