...
class Single_EweiShopV2Page extends shequWebPage
{
	...
	public function getdata()
	{
		...
		if ($_W['ispost']) {
			$condition = ' o.uniacid = :uniacid and m.uniacid = :uniacid and o.deleted=0 and o.addressid<>0 and o.shequid=:shequid and o.isparent=0';
		...
		if ($_W['ispost']) {
			...
			if ($orderinfo['status'] == 1 || $orderinfo['status'] == 0 && $orderinfo['paytype'] == 3) {
				...
				if (!empty($orderinfo['refundid'])) {
					$refund = pdo_fetch('select * from ' . tablename('ewei_shop_order_refund') . ' where id=:id and shequid=:shequid limit 1', array(':id' => $orderinfo['refundid'], ':shequid' => $shequid));
				...
				plog('shequ.exhelper.print.single.dosend', '一键发货 订单号: ' . $orderinfo['ordersn'] . ' <br/>快递公司: ' . $_GPC['expresscom'] . ' 快递单号: ' . $_GPC['expresssn']);
