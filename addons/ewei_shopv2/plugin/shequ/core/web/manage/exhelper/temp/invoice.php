...
class Invoice_EweiShopV2Page extends shequWebPage
{
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				...
				plog('shequ.exhelper.temp.invonice.edit', '修改发货单信息 ID: ' . $id);
			}
			else {
				$data['uniacid'] = $_W['uniacid'];
				$data['shequid'] = $shequid;
				pdo_insert('ewei_shop_exhelper_express', $data);
				$id = pdo_insertid();
				plog('shequ.exhelper.temp.invonice.add', '添加发货单模板 ID: ' . $id);
			}

			if (!empty($data['isdefault'])) {
				pdo_update('ewei_shop_exhelper_express', array('isdefault' => 0), array('type' => 2, 'uniacid' => $_W['uniacid'], 'shequid' => $shequid));
				pdo_update('ewei_shop_exhelper_express', array('isdefault' => 1), array('type' => 2, 'id' => $id, 'shequid' => $shequid));
			}

			show_json(1, array('url' => shequUrl('exhelper/temp/invoice/edit', array('id' => $id))));
			exit();
		}
