...
class Express_EweiShopV2Page extends shequWebPage
{
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				...
				plog('shequ.exhelper.temp.express.edit', '修改快递单信息 ID: ' . $id);
			}
			else {
				...
				$id = pdo_insertid();
				plog('shequ.exhelper.temp.express.add', '添加快递单模板 ID: ' . $id);
			}

			if (!empty($data['isdefault'])) {
				pdo_update('ewei_shop_exhelper_express', array('isdefault' => 0), array('type' => 1, 'uniacid' => $_W['uniacid'], 'shequid' => $shequid));
				pdo_update('ewei_shop_exhelper_express', array('isdefault' => 1), array('type' => 1, 'id' => $id));
			}

			show_json(1, array('url' => shequUrl('exhelper/temp/express/edit', array('id' => $id))));
			exit();
		}
