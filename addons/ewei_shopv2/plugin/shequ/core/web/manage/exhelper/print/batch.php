...
class Batch_EweiShopV2Page extends shequWebPage
{
	public function main()
	{
		...
		$printset = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_exhelper_sys') . ' WHERE uniacid=:uniacid and shequid=:shequid limit 1', array(':uniacid' => $_W['uniacid'], ':shequid' => $shequid));
	...
	public function getdata()
	{
		...
		if ($_W['ispost']) {
			$condition = ' o.uniacid = :uniacid and o.deleted=0 and o.shequid=:shequid and o.isparent=0';
			$condition .= ' and ( o.addressid<>0  or ( o.addressid=0 and o.isverify=0 and o.virtual=0 and o.dispatchtype=1) ) ';
			$paras = array(':uniacid' => $_W['uniacid'], ':shequid' => $shequid);
			...
			if (!empty($_GPC['searchfield']) && !empty($_GPC['keyword'])) {
				...
				if ($searchfield == 'ordersn') {
					...
					$sqlcondition = ' inner join ( select distinct og.orderid from ' . tablename('ewei_shop_order_goods') . ' og left join ' . tablename('ewei_shop_goods') . (' g on g.id=og.goodsid where og.uniacid = \'' . $uniacid . '\' and og.shequid = \'' . $shequid . '\' and (locate(:keyword,g.title)>0)) gs on gs.orderid=o.id');
					$paras[':keyword'] = trim($_GPC['keyword']);
				}
				else {
					if ($searchfield == 'goodssn') {
						$sqlcondition = ' inner join ( select distinct og.orderid from ' . tablename('ewei_shop_order_goods') . ' og left join ' . tablename('ewei_shop_goods') . (' g on g.id=og.goodsid where og.uniacid = \'' . $uniacid . '\' and og.shequid = \'' . $shequid . '\' and (locate(:keyword,g.goodssn)>0)) gs on gs.orderid=o.id');
						$paras[':keyword'] = trim($_GPC['keyword']);
					}
				}
			}
			...
			foreach ($orders as $i => $order) {
				...
				if ($order['dispatchtype'] == 1) {
					...
					$store = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_shequ_store') . ' WHERE id=:id and uniacid=:uniacid and shequid=:shequid limit 1', array(':id' => $order['storeid'], ':uniacid' => $_W['uniacid'], ':shequid' => $shequid));
		...
		if ($_W['ispost']) {
			...
			if (is_array($ordersns)) {
				...
				foreach ($ordersns as $ordersn) {
					pdo_update('ewei_shop_order', array('address_send' => $address_send), array('ordersn' => $ordersn, 'shequid' => $shequid));
				}
	...
	public function getprintTemp()
	{
		...
		if ($_W['ispost']) {
			...
			$tempSender = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_exhelper_senduser') . ' WHERE id=:id and uniacid=:uniacid and shequid=:shequid limit 1', array(':id' => $printUserId, ':uniacid' => $_W['uniacid'], ':shequid' => $shequid));
			$expTemp = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_exhelper_express') . ' WHERE id=:id and uniacid=:uniacid and shequid=:shequid limit 1', array(':id' => $printTempId, ':uniacid' => $_W['uniacid'], ':shequid' => $shequid));
	...
	public function changestate()
	{
		...
		if ($_W['ispost']) {
			...
			foreach ($arr as $i => $data) {
				...
				$ordergood = pdo_fetch('SELECT id,goodsid,printstate,printstate2 FROM ' . tablename('ewei_shop_order_goods') . ' WHERE goodsid=:goodsid and orderid=:orderid and uniacid=:uniacid and shequid=:shequid limit 1', array(':orderid' => $orderid, ':goodsid' => $ordergoodid, ':uniacid' => $_W['uniacid'], ':shequid' => $shequid));

				if ($type == 1) {
					pdo_update('ewei_shop_order_goods', array('printstate' => $ordergood['printstate'] + 1), array('id' => $ordergood['id']));
					$orderprint = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_order_goods') . ' WHERE orderid=:orderid and printstate=0 and uniacid= :uniacid and shequid=:shequid', array(':orderid' => $orderid, ':uniacid' => $_W['uniacid'], ':shequid' => $shequid));
				}
				else {
					if ($type == 2) {
						pdo_update('ewei_shop_order_goods', array('printstate2' => $ordergood['printstate2'] + 1), array('id' => $ordergood['id']));
						$orderprint = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_order_goods') . ' WHERE orderid=:orderid and printstate2=0 and uniacid= :uniacid and shequid=:shequid', array(':orderid' => $orderid, ':uniacid' => $_W['uniacid'], ':shequid' => $shequid));
					}
				}
	...
	public function getorderinfo()
	{
		...
		if ($_W['ispost']) {
			...
			$printTemp = pdo_fetch('SELECT id,type,expressname,express,expresscom FROM ' . tablename('ewei_shop_exhelper_express') . ' WHERE id=:id and type=:type and uniacid=:uniacid and shequid=:shequid limit 1', array(':id' => $temp_express, ':type' => 1, ':uniacid' => $_W['uniacid'], ':shequid' => $shequid));
	...
	public function dosend()
	{
		...
		if ($_W['ispost']) {
			...
			$orderinfo = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_order') . ' WHERE id=:orderid and status>-1 and uniacid=:uniacid and shequid=:shequid limit 1', array(':orderid' => $orderid, ':uniacid' => $_W['uniacid'], ':shequid' => $shequid));
			...
			if ($orderinfo['status'] == 1 || $orderinfo['status'] == 0 && $orderinfo['paytype'] == 3) {
				...
				if (!empty($orderinfo['refundid'])) {
					$refund = pdo_fetch('select * from ' . tablename('ewei_shop_order_refund') . ' where id=:id limit 1', array(':id' => $orderinfo['refundid'], 'shequid' => $shequid));
				...
				plog('shequ.exhelper.print.batch.dosend', '一键发货 订单号: ' . $orderinfo['ordersn'] . ' <br/>快递公司: ' . $_GPC['expresscom'] . ' 快递单号: ' . $_GPC['expresssn']);
