...
class Pages_EweiShopV2Page extends shequWebPage
{
    public function main()
    {
        ...
        $psize = 20;
        $condition = " WHERE uniacid=:uniacid AND shequid=:shequid ";
    ...
    protected function post()
    {
        ...
        if( !empty($id) ) 
        {
            $item = pdo_fetch("SELECT * FROM" . tablename("ewei_shop_quick") . " WHERE id=:id AND uniacid=:uniacid AND shequid=:shequid", array( ":id" => $id, ":uniacid" => $_W["uniacid"], ":shequid" => $_W["shequid"] ));
            if( !empty($item["datas"]) ) 
            {
        ...
        if( $_W["ispost"] ) 
        {
            ...
            {
                pdo_update("ewei_shop_quick", $arr, array( "uniacid" => $_W["uniacid"], "shequid" => $_W["shequid"], "id" => $id ));
                plog("quick.pages.add", "添加购买页面 ID: " . $id . " 标题: " . $arr["title"]);
            }
        ...
        if( !empty($item) ) 
        {
            $url = mobileUrl("quick", array( "id" => $item["id"], "shequid" => $_W["shequid"] ), true);
            $qrcode = m("qrcode")->createQrcode($url);
        }

        $shequid = $_W["shequid"];
        $shequset = p("shequ")->getListUserOne($shequid);
        $shopset = array( "name" => $shequset["shequname"], "logo" => tomedia($shequset["logo"]) );
