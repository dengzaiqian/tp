...
class Dispatch_EweiShopV2Page extends shequWebPage
{
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			show_json(1, array('url' => shequUrl('shop/dispatch', array('op' => 'display'))));
		}

		$dispatch = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_dispatch') . (' WHERE id = \'' . $id . '\' and uniacid = \'' . $_W['uniacid'] . '\' and shequid=\'' . $_W['shequid'] . '\''));
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,dispatchname FROM ' . tablename('ewei_shop_dispatch') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid'] . ' AND shequid=' . $_W['shequid']);
	...
	public function displayorder()
	{
		...
		$item = pdo_fetchall('SELECT id,dispatchname FROM ' . tablename('ewei_shop_dispatch') . (' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid'] . ' and shequid=' . $_W['shequid']));
