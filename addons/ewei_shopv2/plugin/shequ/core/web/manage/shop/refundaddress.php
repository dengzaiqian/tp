...
class Refundaddress_EweiShopV2Page extends shequWebPage
{
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			$data['shequid'] = $_W['shequid'];

			if ($data['isdefault']) {
				pdo_update('ewei_shop_refund_address', array('isdefault' => 0), array('uniacid' => $_W['uniacid'], 'shequid' => $_W['shequid']));
			}

			if (!empty($id)) {
				mplog('shop.refundaddress.edit', '修改退货地址 ID: ' . $id);
				pdo_update('ewei_shop_refund_address', $data, array('id' => $id, 'shequid' => $_W['shequid']));
			}
			else {
			...
			show_json(1, array('url' => shequUrl('shop/refundaddress', array('op' => 'display'))));
		}

		if (!empty($id)) {
			$item = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_refund_address') . (' WHERE id = \'' . $id . '\' and uniacid = \'' . $_W['uniacid'] . '\' and shequid=' . $_W['shequid']));
		}
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('ewei_shop_refund_address') . ' where id=:id and uniacid=:uniacid and shequid=:shequid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']));

		foreach ($items as $item) {
			pdo_delete('ewei_shop_refund_address', array('id' => $item['id'], 'shequid' => $_W['shequid']));
	...
	public function setdefault()
	{
		...
		foreach ($items as $item) {
			pdo_update('ewei_shop_refund_address', array('isdefault' => intval($_GPC['isdefault'])), array('id' => $item['id'], 'shequid' => $_W['shequid']));
