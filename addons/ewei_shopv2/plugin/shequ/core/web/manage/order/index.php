...
class Index_EweiShopV2Page extends shequWebPage
{
	...
	protected function selectOrderPrice($day = 0, $is_all = false, $is_avg = false)
	{
		...
		$sql = 'select id,price,openid,' . $time . '  from ' . tablename('ewei_shop_order') . ' where uniacid = :uniacid and shequid = :shequid and ismr=0 and isparent=0  and deleted=0 ' . $where;
		$param = array(':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid'], ':createtime1' => $createtime1, ':createtime2' => $createtime2);
