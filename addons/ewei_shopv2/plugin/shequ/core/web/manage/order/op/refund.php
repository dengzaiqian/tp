...
class Refund_EweiShopV2Page extends shequWebPage
{
	protected function opData()
	{
		...
		$item = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_order') . ' WHERE id = :id and uniacid=:uniacid and shequid = :shequid  Limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid'], 'shequid' => $_W['shequid']));
	...
	public function submit()
	{
		...
		if ($_W['ispost']) {
			...
			if ($refund['status'] < 0 || $refund['status'] == 1) {
				pdo_update('ewei_shop_order', array('refundstate' => 0), array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'shequid' => $_W['shequid']));
			...
			if ($refundstatus == 0) {
				...
				if ($raid == 0) {
					$raddress = pdo_fetch('select * from ' . tablename('ewei_shop_refund_address') . ' where isdefault=1 and uniacid=:uniacid and shequid = :shequid limit 1', array(':uniacid' => $uniacid, ':shequid' => $shequid));
				}
				else {
					$raddress = pdo_fetch('select * from ' . tablename('ewei_shop_refund_address') . ' where id=:id and uniacid=:uniacid and shequid = :shequid limit 1', array(':id' => $raid, ':uniacid' => $uniacid, ':shequid' => $shequid));
				}

				if (empty($raddress)) {
					$raddress = pdo_fetch('select * from ' . tablename('ewei_shop_refund_address') . ' where uniacid=:uniacid and shequid = :shequid order by id desc limit 1', array(':uniacid' => $uniacid, ':shequid' => $shequid));
				}

				unset($raddress['uniacid']);
				unset($raddress['shequid']);
				...
				pdo_update('ewei_shop_order', $order_data, array('id' => $item['id'], 'uniacid' => $uniacid, 'shequid' => $shequid));
				m('notice')->sendOrderMessage($item['id'], true);
			}
				...
				m('notice')->sendOrderMessage($item['id'], true);
				pdo_update('ewei_shop_order', array('refundstate' => 0, 'status' => -1, 'refundtime' => $time), array('id' => $item['id'], 'uniacid' => $uniacid, 'shequid' => $shequid));

				foreach ($goods as $g) {
					$salesreal = pdo_fetchcolumn('select ifnull(sum(total),0) from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid ' . ' where og.goodsid=:goodsid and o.status>=1 and o.uniacid=:uniacid and o.shequid=:shequid limit 1', array(':goodsid' => $g['id'], ':uniacid' => $uniacid, 'shequid' => $shequid));
				...
				pdo_update('ewei_shop_order', array('refundstate' => 0), array('id' => $item['id'], 'uniacid' => $uniacid, 'shequid' => $shequid));
			}
			else {
				if ($refundstatus == 2) {
					$refundtype = 2;
					...
					m('notice')->sendOrderMessage($item['id'], true);
					pdo_update('ewei_shop_order', array('refundstate' => 0, 'status' => -1, 'refundtime' => $time), array('id' => $item['id'], 'uniacid' => $uniacid, 'shequid' => $shequid));
					$goods = pdo_fetchall('SELECT g.id,g.credit, o.total,o.realprice FROM ' . tablename('ewei_shop_order_goods') . ' o left join ' . tablename('ewei_shop_goods') . ' g on o.goodsid=g.id ' . ' WHERE o.orderid=:orderid and o.uniacid=:uniacid and o.shequid = :shequid', array(':orderid' => $item['id'], ':uniacid' => $uniacid, ':shequid' => $shequid));
					$credits = m('order')->getGoodsCredit($goods);
		...
		$refund_address = pdo_fetchall('select * from ' . tablename('ewei_shop_refund_address') . ' where uniacid=:uniacid and shequid = :shequid', array(':uniacid' => $_W['uniacid'], ':shequid' => $shequid));
