...
class Op_EweiShopV2Page extends shequWebPage
{
	public function delete()
	{
		...
		pdo_update('ewei_shop_order', array('deleted' => 1), array('id' => $orderid, 'uniacid' => $_W['uniacid'], 'shequid' => $_W['shequid']));
	...
	protected function opData()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_order') . ' WHERE id = :id and uniacid=:uniacid and shequid = :shequid', array(':id' => $id, ':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']));
	...
	public function changeprice()
	{
		...
		if ($_W['ispost']) {
			if (empty($shequ_user['changepricechecked'])) {
			...
			foreach ($changegoodsprice as $ogid => $change) {
				$og = pdo_fetch('select price,realprice from ' . tablename('ewei_shop_order_goods') . ' where id=:ogid and uniacid=:uniacid and shequid = :shequid limit 1', array(':ogid' => $ogid, ':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']));
			...
			if (!empty($orderupdate)) {
				pdo_update('ewei_shop_order', $orderupdate, array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'shequid' => $_W['shequid']));
			}
			...
			foreach ($changegoodsprice as $ogid => $change) {
				$og = pdo_fetch('select price,realprice,changeprice from ' . tablename('ewei_shop_order_goods') . ' where id=:ogid and uniacid=:uniacid and shequid = :shequid limit 1', array(':ogid' => $ogid, ':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']));
	...
	public function pay($a = array(), $b = array())
	{
		...
		if (!empty($item['virtual']) && com('virtual')) {
			...
			pdo_update('ewei_shop_order', array('status' => 1, 'paytype' => 11, 'paytime' => time()), array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'shequid' => $_W['shequid']));
	...
	public function close()
	{
		...
		if ($_W['ispost']) {
			...
			if (0 < $item['refundstate'] && !empty($item['refundid'])) {
				...
				pdo_update('ewei_shop_order_refund', $change_refund, array('id' => $item['refundid'], 'uniacid' => $_W['uniacid'], 'shequid' => $_W['shequid']));
			}
	...
	public function paycancel()
	{
		...
		if ($_W['ispost']) {
			m('order')->setStocksAndCredits($item['id'], 2);
			pdo_update('ewei_shop_order', array('status' => 0, 'cancelpaytime' => time()), array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'shequid' => $_W['shequid']));
	...
	public function finish()
	{
		...
		if (empty($shequ_user['finishchecked'])) {
			show_json(0, '您没有确认收货的权限!');
		}

		pdo_update('ewei_shop_order', array('status' => 3, 'finishtime' => time()), array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'shequid' => $_W['shequid']));
		...
		//2019-1-9afei
		if (p('bonus')) {
			p('bonus')->checkOrderFinish($item['id']);
		}
		//2019-1-9afei
	...
	public function fetchcancel()
	{
		...
		pdo_update('ewei_shop_order', array('status' => 1, 'finishtime' => 0), array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'shequid' => $_W['shequid']));
	...
	public function sendcancel()
	{
		...
		if ($_W['ispost']) {
			...
			pdo_update('ewei_shop_order', array('status' => 1, 'sendtime' => 0, 'remarksend' => $remark), array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'shequid' => $_W['shequid']));
	...
	public function fetch()
	{
		...
		pdo_update('ewei_shop_order', $d, array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'shequid' => $_W['shequid']));
	...
	public function send()
	{
		...
		if ($_W['ispost']) {
			...
			$time = time();
			pdo_update('ewei_shop_order', array('status' => 2, 'express' => trim($_GPC['express']), 'expresscom' => trim($_GPC['expresscom']), 'expresssn' => trim($_GPC['expresssn']), 'sendtime' => $time), array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'shequid' => $_W['shequid']));
	...
	public function remarksaler()
	{
		...
		if ($_W['ispost']) {
			pdo_update('ewei_shop_order', array('remarksaler' => $_GPC['remark']), array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'shequid' => $_W['shequid']));
	...
	public function changeexpress()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($expresssn)) {
				...
				pdo_update('ewei_shop_order', $change_data, array('id' => $id, 'uniacid' => $_W['uniacid'], 'shequid' => $_W['shequid']));
	...
	public function changeaddress()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				...
				$item = pdo_fetch('SELECT id, ordersn, address FROM ' . tablename('ewei_shop_order') . ' WHERE id = :id and uniacid=:uniacid and shequid = :shequid', array(':id' => $id, ':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']));
				...
				pdo_update('ewei_shop_order', array('address' => $address_array), array('id' => $id, 'uniacid' => $_W['uniacid'], 'shequid' => $_W['shequid']));
