...
class Detail_EweiShopV2Page extends shequWebPage
{
	public function main()
	{
		...
		if ($_W['ispost']) {
			pdo_update('ewei_shop_order', array('remark' => trim($_GPC['remark'])), array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'shequid' => $_W['shequid']));
		...
		$dispatch = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_dispatch') . ' WHERE id = :id and uniacid=:uniacid and shequid = :shequid', array(':id' => $item['dispatchid'], ':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']));
		...
		$refund = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_order_refund') . ' WHERE orderid = :orderid and uniacid=:uniacid and shequid = :shequid order by id desc', array(':orderid' => $item['id'], ':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']));
		$diyformfields = '';
		$showdiyform = false;
		...
		$condition = ' o.uniacid=:uniacid and  o.shequid = :shequid and o.deleted=0';
		...
		if (com('verify')) {
			...
			if (!empty($item['verifyopenid'])) {
				...
				$saler['salername'] = pdo_fetchcolumn('select salername from ' . tablename('ewei_shop_shequ_saler') . ' where openid=:openid and uniacid=:uniacid and shequid = :shequid limit 1 ', array(':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid'], ':openid' => $item['verifyopenid']));
			}

			if (!empty($item['verifystoreid'])) {
				$store = pdo_fetch('select * from ' . tablename('ewei_shop_shequ_store') . ' where id=:storeid limit 1 ', array(':storeid' => $item['verifystoreid']));
			}
