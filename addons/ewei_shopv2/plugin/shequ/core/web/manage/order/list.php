...
class List_EweiShopV2Page extends shequWebPage 
{
	protected function orderData($status, $st) 
	{
		...
		$condition = " o.uniacid = :uniacid and o.shequid = :shequid and o.deleted=0 and o.isparent=0";
		$uniacid = $_W["uniacid"];
		$shequid = $_W["shequid"];
		$paras = $paras1 = array( ":uniacid" => $uniacid, ":shequid" => $shequid );
						...
						else 
						{
							...
							else 
							{
								...
								else 
								{
									...
									{
										$sqlcondition = " left join " . tablename("ewei_shop_shequ_store") . " store on store.id = o.verifystoreid and store.uniacid=o.uniacid";
									}
									else 
									{
			...
			foreach( $list as &$row ) 
			{
				...
				if( com("verify") ) 
				{
					...
					if( !empty($row["verifyopenid"]) ) 
					{
						...
						$shequ_saler = pdo_fetch("select id,salername from " . tablename("ewei_shop_shequ_saler") . " where openid=:openid and uniacid=:uniacid and shequid = :shequid limit 1 ", array( ":uniacid" => $_W["uniacid"], ":shequid" => $_W["shequid"], ":openid" => $row["verifyopenid"] ));
						$saler["salername"] = (isset($shequ_saler["salername"]) ? $shequ_saler["salername"] : "");
						$row["salerinfo"] = ("[" . isset($shequ_saler["id"]) ? $shequ_saler["id"] : "" . "]" . $saler["salername"] . "(" . $row["nickname"] . ")");
					}
					if( !empty($row["verifystoreid"]) ) 
					{
						$row["storeinfo"] = pdo_fetchcolumn("select storename from " . tablename("ewei_shop_shequ_store") . " where id=:storeid limit 1 ", array( ":storeid" => $row["verifystoreid"] ));
					}
					if( $row["isverify"] && is_array($verifyinfo) && empty($row["dispatchtype"]) ) 
					{
						...
						{
							$v["storename"] = pdo_fetchcolumn("select storename from " . tablename("ewei_shop_shequ_store") . " where id=:id limit 1", array( ":id" => $v["verifystoreid"] ));
							...
							$v["salername"] = pdo_fetchcolumn("select salername from " . tablename("ewei_shop_shequ_saler") . " where openid=:openid and uniacid=:uniacid and shequid = :shequid limit 1", array( ":openid" => $v["verifyopenid"], ":uniacid" => $_W["uniacid"], ":shequid" => $_W["shequid"] ));
		...
		if( $searchfield == "member" ) 
		{
			...
			if( !empty($openidArr) ) 
			{
				...
				$orderPrice = pdo_fetch("SELECT count(1) as count,sum(price) as sumprice FROM " . tablename("ewei_shop_order") . " WHERE uniacid = " . $_W["uniacid"] . " AND shequid = " . $shequid . " AND deleted=0 AND isparent=0 AND openid IN (" . $inOpenid . ")" . $priceStatus);
			}
			else 
			{
		...
		else 
		{
			if( $searchfield == "address" ) 
			{
				$orderPrice = pdo_fetch("SELECT count(1) as count,sum(o.price) as sumprice FROM " . tablename("ewei_shop_order") . " o left join " . tablename("ewei_shop_member_address") . " a on o.addressid = a.id WHERE o.uniacid = " . $_W["uniacid"] . "  AND o.deleted=0 AND o.isparent=0 AND o.shequid = " . $shequid . $priceCondition . $statuscondition);
				$totalmoney = $orderPrice["sumprice"];
				$total = $orderPrice["count"];
			...
			else 
			{
				...
				{
					$t = pdo_fetch("SELECT count(DISTINCT(o.id)) as count,sum(o.price) as sumprice FROM " . tablename("ewei_shop_order") . " o " . " left join " . tablename("ewei_shop_order_refund") . " r on r.id =o.refundid " . " left join " . tablename("ewei_shop_shequ_saler") . " s on s.openid = o.verifyopenid and s.uniacid=o.uniacid and s.shequid=o.shequid" . " left join " . tablename("ewei_shop_member") . " sm on sm.openid = s.openid and sm.uniacid=s.uniacid" . " " . $sqlcondition . " WHERE " . $condition . " " . $statuscondition, $paras);
					$total = $t["count"];
		...
		$stores = pdo_fetchall("select id,storename from " . tablename("ewei_shop_shequ_store") . " where uniacid=:uniacid and shequid = :shequid", array( ":uniacid" => $uniacid, ":shequid" => $_W["shequid"] ));
