...
class Index_EweiShopV2Page extends shequWebPage 
{
	...
	public function main() 
	{
		if( mcv("sale.enough") ) 
		{
			header("location: " . shequUrl("sale/enough"));
		}
		else 
		{
			if( mcv("sale.enoughfree") ) 
			{
				header("location: " . shequUrl("sale/enoughfree"));
			}
			else 
			{
				if( mcv("sale.coupon") ) 
				{
					header("location: " . shequUrl("sale/coupon"));
				}
				else 
				{
	...
	public function enoughfree() 
	{
		...
		if( !empty($data) ) 
		{
			$shequdata = m("cache")->get("shequ_sets_" . $_W["shequid"]);
			$data = $shequdata["sale"];
		}
