...
class Category_EweiShopV2Page extends shequWebPage
{
	...
	public function main()
	{
		...
		if (!empty($_GPC['catid'])) {
			foreach ($_GPC['catid'] as $k => $v) {
				$data = array('name' => trim($_GPC['catname'][$k]), 'displayorder' => $k, 'status' => intval($_GPC['status'][$k]), 'uniacid' => $_W['uniacid'], 'shequid' => $_W['shequid']);
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_coupon_category') . (' WHERE uniacid = \'' . $_W['uniacid'] . '\' AND shequid = \'' . $_W['shequid'] . '\' ORDER BY displayorder asc'));
	...
	public function delete()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT id,name FROM ' . tablename('ewei_shop_coupon_category') . (' WHERE id = \'' . $id . '\' AND uniacid=') . $_W['uniacid'] . ' AND shequid=' . $_W['shequid']);
