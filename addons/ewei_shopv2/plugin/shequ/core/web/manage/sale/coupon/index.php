...
class Index_EweiShopV2Page extends shequWebPage
{
	...
	public function main()
	{
		...
		$psize = 20;
		$condition = ' uniacid = :uniacid and shequid=:shequid';
		...
		foreach ($list as &$row) {
			...
			$row['gettotal'] = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_coupon_data') . ' where couponid=:couponid and uniacid=:uniacid and shequid=:shequid limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']));
			$row['usetotal'] = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_coupon_data') . ' where used = 1 and couponid=:couponid and uniacid=:uniacid and shequid=:shequid limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']));
			$row['pwdjoins'] = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_coupon_guess') . ' where couponid=:couponid and uniacid=:uniacid and shequid=:shequid limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']));
			$row['pwdoks'] = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_coupon_guess') . ' where couponid=:couponid and uniacid=:uniacid and shequid=:shequid and ok=1 limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']));
		}

		unset($row);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_coupon') . (' where 1 and ' . $condition), $params);
		$pager = pagination2($total, $pindex, $psize);
		$category = pdo_fetchall('select * from ' . tablename('ewei_shop_coupon_category') . ' where uniacid=:uniacid and shequid=:shequid order by id desc', array(':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']), 'id');
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			$data = array('uniacid' => $_W['uniacid'], 'shequid' => $_W['shequid'], 'couponname' => trim($_GPC['couponname']), 'coupontype' => intval($_GPC['coupontype']), 'catid' => intval($_GPC['catid']), 'timelimit' => intval($_GPC['timelimit']), 'usetype' => intval($_GPC['usetype']), 'returntype' => 0, 'enough' => trim($_GPC['enough']), 'timedays' => intval($_GPC['timedays']), 'timestart' => strtotime($_GPC['time']['start']), 'timeend' => strtotime($_GPC['time']['end']), 'backtype' => intval($_GPC['backtype']), 'deduct' => trim($_GPC['deduct']), 'discount' => trim($_GPC['discount']), 'gettype' => intval($_GPC['gettype']), 'getmax' => intval($_GPC['getmax']), 'credit' => intval($_GPC['credit']), 'money' => trim($_GPC['money']), 'usecredit2' => intval($_GPC['usecredit2']), 'total' => intval($_GPC['total']), 'bgcolor' => trim($_GPC['bgcolor']), 'thumb' => save_media($_GPC['thumb']), 'remark' => trim($_GPC['remark']), 'desc' => m('common')->html_images($_GPC['desc']), 'descnoset' => intval($_GPC['descnoset']), 'status' => intval($_GPC['status']), 'resptitle' => trim($_GPC['resptitle']), 'respthumb' => save_media($_GPC['respthumb']), 'respdesc' => trim($_GPC['respdesc']), 'respurl' => trim($_GPC['respurl']), 'tagtitle' => $_GPC['tagtitle'], 'settitlecolor' => intval($_GPC['settitlecolor']), 'titlecolor' => $_GPC['titlecolor'], 'limitdiscounttype' => intval($_GPC['limitdiscounttype']));
			...
			show_json(1, array('url' => shequUrl('sale/coupon/edit', array('id' => $id, 'tab' => str_replace('#tab_', '', $_GPC['tab'])))));
		}

		$item = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_coupon') . ' WHERE id =:id and uniacid=:uniacid and shequid=:shequid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id, ':shequid' => $_W['shequid']));
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,couponname FROM ' . tablename('ewei_shop_coupon') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid'] . ' AND shequid=' . $_W['shequid']);

		foreach ($items as $item) {
			pdo_delete('ewei_shop_coupon', array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'shequid' => $_W['shequid']));
			pdo_delete('ewei_shop_coupon_data', array('couponid' => $item['id'], 'uniacid' => $_W['uniacid'], 'shequid' => $_W['shequid']));
			mplog('sale.coupon.delete', '删除优惠券 ID: ' . $id . '  <br/>优惠券名称: ' . $item['couponname'] . ' ');
		}

		show_json(1, array('url' => shequUrl('sale/coupon')));
	}
	...
	public function querygoods()
	{
		...
		$params[':deleted'] = 0;
		$params[':shequid'] = $_W['shequid'];
		$condition = ' and uniacid=:uniacid and deleted = :deleted and  shequid=:shequid';
	...
	public function set()
	{
		...
		if ($_W['ispost']) {
			...
			show_json(1, array('url' => shequUrl('sale/coupon/set', array('tab' => str_replace('#tab_', '', $_GPC['tab'])))));
		}
