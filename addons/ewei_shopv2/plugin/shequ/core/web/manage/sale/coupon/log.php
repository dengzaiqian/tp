...
class Log_EweiShopV2Page extends shequWebPage
{
	...
	public function main()
	{
		...
		$psize = 20;
		$condition = ' d.uniacid = :uniacid and d.shequid = :shequid';
		$params = array(':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']);
		$couponid = intval($_GPC['couponid']);

		if (!empty($couponid)) {
			$coupon = pdo_fetch('select * from ' . tablename('ewei_shop_coupon') . ' where id=:id and uniacid=:uniacid and shequid=:shequid limit 1', array(':id' => $couponid, ':uniacid' => $_W['uniacid'], ':shequid' => $_W['shequid']));
