...
// class Temp_EweiShopV2Page extends PluginWebPage
{
	public function main()
	{
		global $_W;
		global $_GPC;
		$globalData = p("diyform")->globalData();
		extract($globalData);
		$page = (empty($_GPC['page']) ? '' : $_GPC['page']);
		$pindex = max(1, intval($page));
		$psize = 20;
		$kw = (empty($_GPC['keyword']) ? '' : $_GPC['keyword']);
		$items = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_diyform_type') . ' WHERE shequid='.$_W['shequid'].' and uniacid=:uniacid and title like :name order by id desc limit ' . (($pindex - 1) * $psize) . ',' . $psize, array(':name' => '%' . $kw . '%', ':uniacid' => $_W['uniacid']));
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_diyform_type') . ' WHERE shequid='.$_W['shequid'].' and uniacid=:uniacid and title like :name order by id desc ', array(':uniacid' => $_W['uniacid'], ':name' => '%' . $kw . '%'));
		...
		foreach ($items as $key => &$value) {
			...
			$value['datacount3'] = p("diyform")->getCountGoodsUsed($value['id']);
	...
	protected function post()
	{
		...
		if (!empty($id)) {
			$item = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_diyform_type') . ' WHERE shequid='.$_W['shequid'].' and id=:id and uniacid=:uniacid ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
			...
			$datacount3 = p("diyform")->getCountGoodsUsed($id);
		}

		if ($_W['ispost']) {
			$data = p("diyform")->getInsertDataByAdmin();
			$insert = array('uniacid' => $_W['uniacid'], 'cate' => intval($_GPC['cate']), 'title' => trim($_GPC['tp_title']), 'fields' => iserializer($data), 'savedata' => intval($_GPC['savedata']),'shequid'=>$_W['shequid']);
	...
	public function delete()
	{
		...
		$types = pdo_fetchall('SELECT id,title  FROM ' . tablename('ewei_shop_diyform_type') . ' WHERE shequid='.$_W['shequid'].' and id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		$errmsg = '';
