...
class Register_EweiShopV2Page extends PluginMobileLoginPage
{
	public function main()
	{
		global $_W;
		global $_GPC;
		$set = $_W['shopset']['shequ'];
        
        $shequid = intval($_GPC['shequid']);
        $condition = "1";
        if(!empty($shequid)){
            $condition .= " and  ";
            $shequ_rule = pdo_fetch('select * from ' . tablename('rule') . ' where shequid=:shequid and uniacid=:uniacid limit 1', array(':shequid' => $shequid, ':uniacid' => $_W['uniacid']));
        }
        
        if (empty($set['apply_openmobile']) || (!empty($shequid) && empty($shequ_rule['status']))) {
			$this->message('未开启社区入驻申请', '', 'error');
		}
        
        
		$reg = pdo_fetch('select * from ' . tablename('ewei_shop_shequ_reg') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']));
		$user = false;

		if (!empty($reg['status'])) {
			$user = pdo_fetch('select * from ' . tablename('ewei_shop_shequ_user') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']));
		}
		...
		if ($_W['ispost']) {
			...
			$usercount1 = pdo_fetchcolumn('select count(1) from ' . tablename('ewei_shop_shequ_reg') . (' where ' . $where1 . ' limit 1'), $params1);
			$where2 = ' username=:username';
			$where2 .= ' and uniacid=:uniacid';
			$params2 = array(':username' => $uname);
			$params2['uniacid'] = $_W['uniacid'];
			$usercount2 = pdo_fetchcolumn('select count(1) from ' . tablename('ewei_shop_shequ_account') . (' where ' . $where2 . ' limit 1'), $params2);
			...
			if (empty($reg)) {
				$data['applytime'] = time();
				pdo_insert('ewei_shop_shequ_reg', $data);
				$regid = pdo_insertid();
				$user = array(
                    'uniacid' => $_W['uniacid'],
                    'shequname' => trim($data['shequname']),
				...
				pdo_update('ewei_shop_shequ_reg', $data, array('id' => $reg['id']));
				$user = array(
                    'uniacid' => $_W['uniacid'],
                    'shequname' => trim($data['shequname']),
                    'salecate' => trim($data['salecate']),
				...
				pdo_update('ewei_shop_shequ_user', $user, array('regid' => $reg['id'], 'uniacid' => $_W['uniacid']));
			}

			$this->model->sendMessage(array('shequname' => $data['shequname'], 'salecate' => $data['salecate'], 'realname' => $data['realname'], 'mobile' => $data['mobile'], 'applytime' => time()), 'shequ_apply');
			show_json(1);
		}
	...
	public function notice()
	{
		global $_W;
		$set = $_W['shopset']['shequ'];
		include $this->template('shequ/register_notice');
	}
