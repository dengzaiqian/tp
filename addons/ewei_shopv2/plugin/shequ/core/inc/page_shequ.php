...
require EWEI_SHOPV2_PLUGIN . 'shequ/core/inc/common.php';
class ShequWebPage extends PluginWebPage
{
	public $pluginname;
	public $model;
	public $plugintitle;
	...
	{
		global $_W;
		if ($_W['shequisfounder'] != '1' && $_W['routes'] != 'shop') {
			$perm = mcv($_W['routes']);
			$perm_type = array();

			if (p('shequ')) {
				$perm_type = p('shequ')->getLogTypes(true);
			}
			...
			if (p('shequ')) {
				$is_xxx = p('shequ')->check_xxx($_W['routes']);
			}
