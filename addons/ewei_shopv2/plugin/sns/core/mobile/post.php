...
class Post_EweiShopV2Page extends SnsMobilePage
{
	public function main()
	{
		...
		if (!$isManager && !$isSuperManager) {
			...
			$canpost = !is_error($check);
                ...
                //判断浏览本帖的用户不是发帖人
                if($member["openid"]!=$_W['openid']){       //查看本帖的用户不是发帖人
                    $is_me=0;
                }else{                                      //查看本帖的用户是发帖人
                    $is_me=1;
                }
	...
	public function getlist()
	{
		...
		$total = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_sns_post') . (' where 1 ' . $condition), $params);
                ...
                /*************查询自己和回复自己的评论和start******************/
                //查询该话题采纳记录
                $adopt_log=pdo_fetch('select * from ' . tablename('ewei_shop_sns_adopt_log') . ' where uniacid=:uniacid and hid=:id  limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $pid));
                //查询发帖者
                $post_owner=pdo_fetch('select openid from ' . tablename('ewei_shop_sns_post') . ' where uniacid=:uniacid and id=:id  limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $pid));
                //查询评论者和发帖者
                $sql2 = 'select id from ' . tablename('ewei_shop_sns_post') . ('  where `uniacid` = :uniacid and bid=:bid and pid=:pid and openid=:openid and `deleted`=0 ORDER BY createtime desc ');
		$pinglun_list = pdo_fetchall($sql2, array(':uniacid' => $_W['uniacid'], ':pid' => $pid, ':bid' => $bid,':openid'=>$_W['openid']));
                $ids=array();
                ...
                //回复评论者的评论
                if(!empty($id_str)){
                        $sql3 = 'select id from ' . tablename('ewei_shop_sns_post') . '  where `uniacid` = :uniacid and bid=:bid and pid=:pid and openid !=:openid and `deleted`=0  ';
                            $sql3.=' and rpid in ('.$id_str.')';
                        $huifu_list = pdo_fetchall($sql3, array(':uniacid' => $_W['uniacid'], ':pid' => $pid, ':bid' => $bid,':openid'=>$_W['openid']));
                        foreach ($huifu_list as $k => $v) {
                            $ids[]=$v["id"];
                        }
                }
                /*************查询自己和回复自己的评论和end******************/
		...
		foreach ($list as $key => &$row) {
			...
			$row['isManager'] = $this->model->isManager($row['bid'], $row['openid']);
                        //
                        $row["is_adopt"]=0;                                     //是否被采纳
                        $row["is_has_adopt"]=0;                                 //是否有采纳记录
                        if(!empty($adopt_log)){                                 //存在采纳记录
                            $row["is_has_adopt"]=1; 
		}
                        $row["adopt_status"]=0;  
                        if($row["id"]==$adopt_log["pid"]){                      //该评论被采纳
                            $row["is_adopt"]=1;
                            $row["adopt_status"]=$adopt_log["status"];  
                        }
                        $row["is_owner"]=0;  
                        if($post_owner["openid"]==$row["openid"]){                      //该评论是发帖人
                            $row["is_owner"]=1;
                        }

                        if(!in_array($row["id"], $ids)&&$post_owner["openid"]!=$_W["openid"]){          //1.不是发帖者  2.不是回复自己的评论
                            unset($list[$key]);
                        }
		}
	...
	public function alltop()
	{
		...
		global $_GPC;
			...
			show_json(0, '未登录');
                ...
                $pid = intval($_GPC['pid']);
                $post=pdo_fetch('select id,pid,openid from ' . tablename('ewei_shop_sns_post') . ' where uniacid=:uniacid and id=:id and deleted=0 limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $pid));
                if(empty($post)){
                    show_json(0, '未找到话题!');
                }
                $ppost=pdo_fetch('select id,reward_credit,openid from ' . tablename('ewei_shop_sns_post') . ' where uniacid=:uniacid and id=:id and deleted=0 limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $post["pid"]));
                if($ppost["openid"]!=$_W["openid"]){        //不是发帖人
                    show_json(0, '发帖人才能进行采纳!');
                }
                $log=array(
                    'uniacid'=>$_W['uniacid'],
                    'pid'=>$post["id"],                                         //评论id    采纳的评论
                    'hid'=>$ppost["id"],                                        //话题id    
                    'createtime'=>time(),                                       //采纳时间
                    'reward_credit'=>$ppost["reward_credit"],                   //奖励积分
                    'openid'=>$post['openid'],                                  //被采纳的用户openid
                    'status'=>1,                                                //采纳状态（1：已采纳，2：已赠送积分）
                );
                //添加记录
                $res=pdo_insert('ewei_shop_sns_adopt_log', $log);
                if(!$res){
                    show_json(0, '采纳失败，请稍后重试!');
                }
                show_json(1, $res);
        }

        public function give_credit(){
                global $_W;
		global $_GPC;
                if (!$this->islogin) {
			show_json(0, '未登录');
                ...
                $ppost=pdo_fetch('select id,reward_credit,openid from ' . tablename('ewei_shop_sns_post') . ' where uniacid=:uniacid and id=:id and deleted=0 limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $post["pid"]));
                if($ppost["openid"]!=$_W["openid"]){        //不是发帖人
                    show_json(0, '发帖人才能进行操作!');
                }
                
                $update=array(
                    'status'=>2,                                                //采纳状态（1：已采纳，2：已赠送积分）
                    'give_time'=>time()                                          //赠送积分时间
                );
                $res=pdo_update('ewei_shop_sns_adopt_log', $update, array('pid' => $pid));
                //赠送评论者积分
                m('member')->setCredit($post['openid'], 'credit1', $ppost["reward_credit"], '采纳评论赠送积分 ' . $ppost["reward_credit"]);
                if(!$res){
                    show_json(0, '操作失败，请稍后重试!');
                }
                show_json(1, $res);
        }
}
